...
CREATE TABLE ExhaladoRE (
  idExhaladoRE INTEGER GENERATED BY DEFAULT AS IDENTITY,
  medicion NUMBER(7, 2) NOT NULL,
  idVO2MAX INTEGER NOT NULL,
  PRIMARY KEY (idExhaladoRE),
  FOREIGN KEY (idVO2MAX) REFERENCES VO2MAX(idVO2MAX)
    ON DELETE CASCADE
);

CREATE TABLE InhaladoRE (
  idInhaladoRE INTEGER GENERATED BY DEFAULT AS IDENTITY,
  medicion NUMBER(7, 2) NOT NULL,
  idVO2MAX INTEGER NOT NULL,
  PRIMARY KEY (idInhaladoRE),
  FOREIGN KEY (idVO2MAX) REFERENCES VO2MAX(idVO2MAX)
    ON DELETE CASCADE
);

CREATE TABLE SesionEjercicio (
  idSesion INTEGER GENERATED BY DEFAULT AS IDENTITY,
  estado            VARCHAR2(75) DEFAULT 'INICIADO',
  fechaInicio       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  fechaFin          TIMESTAMP,
  noPasos           INTEGER DEFAULT 0,
  metaPasos         INTEGER DEFAULT 0,
  metaCalorias      INTEGER DEFAULT 0,
  caloriasQuemadas  NUMBER(5,2) DEFAULT 0.00,
  idUsuario         INTEGER NOT NULL,
...
CREATE OR REPLACE TRIGGER tg_setFechaFinSesion
FOR UPDATE OF ESTADO ON SESIONEJERCICIO
COMPOUND TRIGGER
  v_idSE INTEGER;
  ...
  AFTER STATEMENT IS
    BEGIN
      UPDATE SESIONEJERCICIO SET 
        fechaFin = CURRENT_TIMESTAMP 
        WHERE idSesion = v_idSE AND estado = 'FINALIZADO';
  END AFTER STATEMENT;
END tg_setFechaFinSesion ;
